...
function tokenVigente() {
    return !!(App.accessToken && App.tokenExpira && Date.now() < App.tokenExpira);
}
...
function sesionLocalVigente() {
    return localStorage.getItem('sesionActiva') === '1' && (ultimaValidacionVigente() || tokenExpiraVigente());
}
        ...
        callback: (tokenResponse) => {
            if (tokenResponse && tokenResponse.access_token) {
                ...
                if (!App.timerRenovacion) {
                    App.timerRenovacion = setInterval(validarSesionPeriodicamente, 120 * 60 * 1000);
                }
...
async function entrarApp() {
    ...
    if (navigator.onLine && App.tokenClient && localStorage.getItem('userEmail')) {
        try {
            App.tokenClient.requestAccessToken({
                prompt: '',
...
window.validarCredenciales = function () {
    if (!App.tokenClient) {
        toast('Sistema de autenticación no disponible.', 'warning', 4000);
        return;
    }
    App.tokenClient.requestAccessToken({
